...
const LoginPage = () => {
  ...
  const loginEndpoint = useMemo(() => resolveUrl('/auth/login'), []);
  const registerEndpoint = useMemo(() => resolveUrl('/auth/register'), []);
  const adminLoginEndpoint = useMemo(() => resolveUrl('/admin/login'), []);
  const adminProductsEndpoint = useMemo(() => resolveUrl('/admin/products'), []);
  ...
  const fetchAdminProducts = useCallback(async () => {
    try {
      ...
      setAdminError(err instanceof Error ? err.message : 'Unable to load products');
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = activeTab === 'signin' ? loginEndpoint : registerEndpoint;
      const body =
        activeTab === 'signin'
          ? { email, password }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data.message ||
            data.error ||
            `Failed to ${activeTab === 'signin' ? 'sign in' : 'create account'}`,
        );
      }
      ...
      setSuccessMessage(
        data.message ??
          (activeTab === 'signin' ? 'Logged in successfully' : 'Account created successfully'),
      );
      setStatus({
        authenticated: true,
      ...
      const message =
        err instanceof Error
          ? err.message
          : `Unable to complete ${activeTab === 'signin' ? 'sign-in' : 'registration'}`;
      setError(message);
    } finally {
  ...
  const handleAdminLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data: ApiError = await response.json().catch(() => ({}));
        throw new Error(data.message || data.error || 'Unable to open admin console');
      }
      ...
      setAdminError(err instanceof Error ? err.message : 'Unable to sign in as admin');
  ...
  const handleAddProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setAdminError(err instanceof Error ? err.message : 'Unable to add product');
    } finally {
  ...
  const handleRemoveProduct = async (productId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data: ApiError = await response.json().catch(() => ({}));
        throw new Error(data.message || data.error || 'Unable to remove product');
      }
      ...
      setAdminError(err instanceof Error ? err.message : 'Unable to remove product');
    } finally {
      setProductActionLoading(false);
    }
  };
        ...
        <div className="bg-white/4 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
              ...
              {isAdminAuthenticated && (
                <div className="space-y-6">
                  <div>
                    ...
                    <form onSubmit={handleAddProduct} className="mt-4 grid gap-4">
                      <input
                        type="text"
          ...
          <div className="mt-6 rounded-lg border border-white/10 bg-white/5 p-4 text-xs">
            <p className="text-white/70 mb-1 flex items-center gap-2">
              Session
              {isStatusLoading && <span className="text-[10px] uppercase tracking-[0.3em]">…</span>}
            </p>
